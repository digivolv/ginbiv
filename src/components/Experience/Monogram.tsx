...
export default function Monogram({ onReady }: MonogramProps) {
  ...
  useEffect(() => {
    ...
    // SVG Y increases downward; negating scale.y inverts the axis so letter
    // silhouettes read right-side-up. Three.js detects the negative determinant
    // and flips the front-face winding test automatically — no geometry mutation needed.
    g.scale.set(cfg.scale, -cfg.scale, cfg.scale)
    g.position.set(
